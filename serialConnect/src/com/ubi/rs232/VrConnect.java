...
public class VrConnect {
	...
	public boolean PortConnect(String filePath, Label label) {
		...
		connectFlag = vract.connection(portNameValue, speed, databits, stopbits, parityEven);
		return connectFlag;
	}
	...
	public void systemClose() {
		...
		port = vract.getPort();
		in = vract.getIn();
		bufferedReader = vract.getBufferedReader();
		try {
			if (in != null) {
				in.close();
			}
	...
	public void portGetValue() {
		...
		TimerTask m_task = new TimerTask() {
			@Override
			public void run() {
				String message = vract.getSerialValue();
