...
public class MwConnect {

	private MwAction mwact;
	...
	public boolean PortConnect(String filePath, Label label) {
		...
		connectFlag = mwact.connection(portNameValue, speed, databits, stopbits, parityEven);
		return connectFlag;
	}
	...
	public void systemClose() {
		...
		port = mwact.getPort();
		in = mwact.getIn();
		bufferedReader = mwact.getBufferedReader();
		try {
			if (in != null) {
				in.close();
			}
	...
	public void portGetValue() {
		...
		TimerTask m_task = new TimerTask() {
			@Override
			public void run() {
				String message = mwact.getSerialValue();
